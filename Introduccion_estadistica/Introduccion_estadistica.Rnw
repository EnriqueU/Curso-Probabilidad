...
y mide la variabilidad entre las observaciones y estima la \texttt{varianza de la poblaci\'on} $\sigma^2 = Var(X)$.
...
Para probar esto supongamos que $\mu = E(X) = 0$, entonces $EX_i^2 = VarX_i = \sigma^2$, as\'i 

\[
E\overline{X} = Var\overline{X} = \sigma^2/n
\]
...
\[
Es^2 = \dfrac{E\displaystyle\sum X_i^2 - nE\overline{X}^2}{n -1} = \dfrac{n\sigma^2 - \sigma^2}{n - 1} = \sigma^2.
\]
...
Desde que $E(Y_i) = 0$, tenemos $E(s_{X}^2) = E(s_{Y}^2) = \sigma_{Y}^2 = \sigma_{X}^2$. De manera similar que para  $\overline{X}$ se puede mostrar que bajo ciertas condiciones, la varianza muestral y la desviaci\'on est\'andar muestral son consistentes y cumplen la propiedad de normalidad as\'intotica.
